...
def proses_pertanyaan (pertanyaan):
  ...
  exclude_stopword = ['setelah', 'telah', 'sebelum', 'belum', 'kedua', 'dua', 'mengapa']
  ...
  # Proses Stemming, Stopword Removal, dan Tokenize
  hasil = []
  hasil.append(word_tokenize((stopWordRemover.remove((stemmer.stem(pertanyaan))))))

  # Proses Pos Tagging(ramshel dan manual tag)
  pos_tag = pd.DataFrame(columns=['kata','tag'])

  ct = CRFTagger()
  ct.set_model_file(r'F:\Documents\Semester_8\Tugas_Akhir\import\with_STEMMING\all_indo_man_tag_corpus_model.crf.tagger')
  pos = ct.tag_sents(hasil)
  pos = pos[0]

  for kata_pos, tag_pos in pos:
    for kata, ganti, tag in import_kelola_data.katatag:
      if kata_pos == kata:
        kata_pos = ganti;
        tag_pos = tag;
...
def identifikasi_kunci(hasil_tag):
  ...
  for kata, tag in hasil_tag:
    if (tag == "NN" or tag == "OD" or tag == "NNP"):
